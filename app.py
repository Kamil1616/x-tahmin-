...
def search_teams(query):
    if len(query) < 3:
        return []
    try:
        resp = requests.get(f"{BASE_URL}/search/teams?q={query}", headers=HEADERS, timeout=10)
...
st.title("Futbol Tahmin Aracı (Son 6 Maç)")

st.markdown("Takım isimlerini Türkçe veya İngilizce yazın (örn: galatasaray, fenerbahce, besiktas, manchester city)")

col1, col2 = st.columns(2)

with col1:
    home_q = st.text_input("Ev Sahibi Takım Ara", key="home_search")
    ...
    home_sel = st.selectbox("Ev Sahibi", home_options, key="home_select")

with col2:
    away_q = st.text_input("Deplasman Takım Ara", key="away_search")
    ...
    away_sel = st.selectbox("Deplasman", away_options, key="away_select")

if st.button("Tahmin Oluştur", type="primary"):
    if home_sel == "Sonuç yok" or away_sel == "Sonuç yok":
        st.warning("Lütfen iki takımı da aratıp seçin.")
    else:
        ...
        if home_entity is None or away_entity is None:
            st.error("Takım bilgileri alınamadı, tekrar aratın.")
        else:
            with st.spinner("Veriler çekiliyor..."):
                ...
                if h_stats is None or a_stats is None:
                    st.warning("Yeterli maç verisi yok (en az 3 maç lazım).")
                else:
